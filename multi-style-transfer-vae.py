...
# # #TODO: use cpu
# # parser.add_argument('--gpu_mode', type=bool, default=False)  # Set gpu_mode to False
...
# matrix.load_state_dict(torch.load(opt.matrixPath, map_location=device))
...
# # matrix.load_state_dict(torch.load(opt.matrixPath, map_location=device, weights_only=True))
...
#             t1 = time.time()
#             #print("===> Processing: %s || Timer: %.4f sec." % (str(i), (t1 - t0)))
#             print("===> Processing: %s || Timer: %.4f sec." % (str("going"), (t1 - t0)))
...
def eval():
    ...
    # Added check for image file types
    valid_extensions = ('.jpg', '.jpeg', '.png', '.bmp', '.gif')
    for cont_file in os.listdir(content_path):
        if cont_file.lower().endswith(valid_extensions):
            for ref_file in os.listdir(ref_path):
                if ref_file.lower().endswith(valid_extensions):
                    t0 = time.time()
                    content = Image.open(os.path.join(content_path, cont_file)).convert('RGB')
                    ref = Image.open(os.path.join(ref_path, ref_file)).convert('RGB')
                    ...
                    t1 = time.time()
                    print("===> Processing: %s with %s || Timer: %.4f sec." % (cont_file, ref_file, t1 - t0))
